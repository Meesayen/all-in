...
GameServer.prototype._handleDeviceConnection = function(socket, data) {
	...
	if (game === undefined) {
		socket.emit('game:wrongtoken', { message: 'Wrong Token' });
	} else {
		if(game.getAvailableSeats() == 0) {
			socket.emit('game:lobbyfull', { message: 'The Lobby is currently full.<br>Only 4 players at once are allowed to join in.' });
		} else {
...
GameServer.prototype._handleDeviceDisconnection = function(socket, data) {
	var game = this.games[data.token];
	if (game === undefined) {
		socket.emit('game:wrongtoken', { message: 'Wrong Token' });
	} else {
