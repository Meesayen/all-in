...
Game.prototype.checkReadyPlayers = function() {
	...
	if(ready >= this.MIN_PLAYERS && this._getCurrentPhase() == this.GAME_PHASES.LOBBY) {
		this.socket.emit('game:ready-to-play');
		this._changePhase(this.GAME_PHASES.IDLE);
	}
	if(ready < this.MIN_PLAYERS && this._getCurrentPhase() == this.GAME_PHASES.IDLE) {
...
Game.prototype._getCurrentPhase = function() {
