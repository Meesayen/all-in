...
export let scripts = (src, dest) => {
  return function _scripts() {
    return gulp.src(src, { since: gulp.lastRun(_scripts) })
      .pipe(gulp.$.babel({
...
export let styles = (src, dest) => {
  return function _styles() {
    return gulp.src(src, { since: gulp.lastRun(_styles) })
      .pipe(gulp.$.less())
      .pipe(gulp.dest(dest))
      .pipe(gulp.$.livereload());
  };
...
export let html = (src, dest) => {
  return function _html() {
    return gulp.src(src, { since: gulp.lastRun(_html) })
